...
function CurrentLocation({ dateDay, hour }) {
  ...
  // console.log(weatherData);
    ...
    return (
        <div className='w-[90%] sm:w-[80%] md:w-[80%] h-full flex flex-col justify-center gap-2 m-auto my-5 sm:my-10 md:my-15 text-[clamp(0.8rem,_1.2rem_+_0.8vw,_1.2vw)]'>
          {error && (
            <div className='fixed inset-0 bg-green bg-opacity-20 flex items-center justify-center z-50'>
              <div className='bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-lg max-w-md mx-4'>
                ...
                <button onClick={() => setError(null)} className='mt-4 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded'>
                  Cerrar
                </button>
              </div>
            </div>
          )}
          <h2 className='text-[clamp(1.2rem,_1.8rem_+_1vw,_4vw)] italic text-center'><strong>Datos del clima en tu ubicación:</strong></h2>
          <section className='w-full flex flex-col sm:flex-col md:flex-row items-center justify-around gap-2'>
            <div className='w-[100%] sm:w-[80%] md:w-auto flex flex-col gap-1 text-center sm:text-center md:text-left'>
            ...
            <figure className='bg-[#32AC90] w-[60%] sm:w-[40%] md:w-[20%] h-[12%] flex items-center'>
